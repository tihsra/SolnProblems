...
public class Main{
	...
	public static void solve(int n, int coins[], int sum){
		
		int memo[][] = new int[n][sum+1];

		for(int subArr[] : memo) Arrays.fill(subArr,-1);

		ps(solver(n,coins,sum,0,memo));
	}

	public static int solver(int n, int coins[], int sum, int i, int memo[][]){

		if(sum==0) return 0;

		if(sum<0 || i>=n) return IMAX;

		if(memo[i][sum]!=-1) return memo[i][sum];
		
		int take = IMAX;
		if(sum-coins[i]>=0) {
			take = solver(n,coins,sum-coins[i],i,memo);
			if(take!=IMAX) take+=1;
			}
		int notTake = solver(n,coins,sum,i+1,memo);
