...
public static void solve(String a, String b){
		...
		for(i=0;i<str.length();i++){
			...
			while(a.charAt(j)!=str.charAt(i)){
				ans.append(a.charAt(j));
				j++;
			}

			while(b.charAt(k)!=str.charAt(i)){
	...
	public static int[][] solver(String a, String b){
		...
		int memo[][] = new int[a.length()+1][b.length()+1]; //  the resonsing is that we would have to manually write base case if we use a.lngth and b.length but here no need
