...
public class Main{
	...
	public static void solve(int n, int arr[], int times){
		...
		ps(solver(n,arr,times,0,1)); 

	}

	public static int solver(int n, int arr[], int times, int idx, int buy){
		...
		if(buy==1){  //denotes we could buy
			...
			int buyIt = solver(n,arr,times,idx+1,0)-arr[idx];
			int notBuyIt = solver(n,arr,times,idx+1,1);

			return Math.max(buyIt,notBuyIt);

		}

		int sellIt = solver(n,arr,times-1,idx+1,1)+arr[idx];
		int notSellIt = solver(n,arr,times,idx+1,0);
