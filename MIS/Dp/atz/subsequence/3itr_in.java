// The code only work for positive integer value of array

// Incomplete : Make it workable for -ve alrray elements as well
...
public class Main{
	...
	public static void solve(int n, int arr[], int sum){
			
			boolean memo[] = solver(n,arr,sum/2);

			int diff = IMAX;

			for(int i=0;i<=(sum/2);i++){

				if(memo[i]) diff = Math.min(diff,sum-2*i);
	...
	public static boolean[] solver(int n, int arr[], int sum){
		...
		for(int i=1;i<=n;i++){
			for(int j=0;j<=sum;j++){
				...
				boolean tk = false;
				if(j-arr[i-1]>=0) tk = memo[i-1][j-arr[i-1]];

				boolean nk = memo[i-1][j];
