...
public class Main{
	...
	public static void solve(int n, int arr[],int fee){

		ps(solver(n,arr,fee)); 

	}

	public static int solver(int n, int arr[], int fee){  
		...
		for(int i=1;i<=n;i++){
			for(int j=0;j<2;j++){ // 1 for buy, 0 for sell
				if(j==1){

					int buyIt = memo[i-1][0]-arr[i-1] ;
					int notBuyIt = memo[i-1][1];

					memo[i][j] = Math.max(buyIt,notBuyIt);
 
				}
				else{

					int sellIt = memo[i-1][1]+arr[i-1]-fee;
					int notSellIt = memo[i-1][0];

					memo[i][j] = Math.max(sellIt,notSellIt);
