...
public class Main{
	...
	public static void solve(int n, int coins[], int sum){
		...
		ps(solver(n,coins,sum));
	
	}

	public static int solver(int n, int coins[], int sum){

		int memo[][] = new int[n][sum+1];
		for(int i[] : memo) Arrays.fill(i,IMAX);
		...
		for(int i=1;i<=sum;i++){
			for(int j=0;j<n;j++){
				if(i-coins[j]>=0) {
					if(memo[j][i-coins[j]]!=IMAX) memo[j][i] = Math.min(memo[j][i],1+memo[j][i-coins[j]]);
				}
			}
		}
