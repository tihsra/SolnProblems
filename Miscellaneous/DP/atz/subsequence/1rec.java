...
public class Main{
	...
	public static void solve(int n, int k, int arr[]){
			...
			System.out.println(solver(n,k,arr,0));

	}

	public static boolean solver(int n, int k, int arr[], int idx){

		if(k==0) return true;

		if(idx>=n||k<0) return false;
		
		boolean inc = solver(n,k-arr[idx],arr,idx+1);
		boolean nic = solver(n,k,arr,idx+1);
