...
public class Main{
	...
	public static int solver(int n, int k, int arr[], int idx, int memo[][]){
		...
		if(k<0||idx>=n) return IMAX/2;

		if(memo[idx][k]!=-1)  return memo[idx][k];
		
		int inc = 1+solver(n,k-arr[idx],arr,idx,memo);
		int nic = solver(n,k,arr,idx+1,memo);

		memo[idx][k] = Math.min(inc,nic);
