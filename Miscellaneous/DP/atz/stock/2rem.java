...
public class Main{
	...
	public static int solver(int n, int arr[], int bos, int i, int memo[][]){  

		if(i>=n) return 0;

		if(memo[i][bos]!=-1) return memo[i][bos];

		if(bos==1){ // buy

			int buyIt = solver(n,arr,0,i+1,memo)-arr[i];
			int notBuyIt = solver(n,arr,1,i+1,memo);

			memo[i][bos] = Math.max(buyIt, notBuyIt);

		}

		else{
			
			int sellIt = solver(n,arr,1,i+1,memo)+arr[i];
			int notSellIt = solver(n,arr,0,i+1,memo);

			memo[i][bos] = Math.max(sellIt, notSellIt);
		}
